import sys
sys.path.append("..")
# from utils import vtt_latent as vl
...
from utils.training_utils import weight_init

import random

from stable_baselines3.common.policies import ActorCriticCnnPolicy
from utils.random_overlay import random_overlay
...
task_set = ["HandManipulateBlockRotateZFixed-v1", "HandManipulateEggRotateFixed-v1", "HandManipulatePenRotateFixed-v1", "HandManipulatePenRotate-v1", "HandManipulatePenRotateFixedInst-v1"]
...
class InfoNCELoss(nn.Module):
    """ InfoNCE loss
    contrastive learning between 2 batch of embedding q and k
    """
    def __init__(self, repr_dim, feature_dim, use_topk, topk, use_time, temp=0.1):
        super().__init__()
        self.temp = temp
    ...
    def contrastive_loss(self, q, k):
        # k stop gradient，q 是需要更新的部分(obs_image obs_tactile alternate)
        # 不能在外面求 logits，因为 k 没 detach ..
        if(self.use_topk):
            k = k.detach() # freeze parameters in k
            logits = torch.einsum('nc,mc->nm', [q, k]) / self.temp
            with torch.no_grad():
                k_logits = torch.einsum('nc,mc->nm', [k, k]).detach()
                if not self.use_time:
                    _, topk_index = torch.topk(k_logits, self.topk, dim=1)
                else:
                    time_neighbors = torch.zeros([q.shape[0], self.topk], device=q.device)
                    for i in range(q.shape[0]):
                        for j in range(self.topk):
                            if(i - j >= 0):
                                time_neighbors[i,j] = i - j
                            else:
                                time_neighbors[i,j] = i
                    topk_index = time_neighbors.to(torch.int64)
                # print('?? ', topk_index)
                topk_mask = torch.zeros_like(k_logits)
                topk_mask.scatter_(1, topk_index, 1)
                topk_mask = topk_mask.bool().detach()
            # print('topk_mask shape:', topk_mask.shape,logits.shape);exit(0)
            contrastive_loss = - torch.log( (F.softmax(logits, dim=1) * topk_mask).sum(1) ).mean()
        else: 
            logits = torch.einsum('nc,mc->nm', [q, k]) / self.temp
    ...
    def forward(self, q, k):
        # print('q, k require: ', q.requires_grad, k.requires_grad)
        q = self.projector(q)
        k = self.projector(k)
...
class CNNEncoder(nn.Module):
    def __init__(self, in_channels, encoder_dim,num_tactiles,frame_stack = 1, key='image', env_name='Door', structure="drqv2"):
        super().__init__()
        ...
        if(self.structure == 'drqv2'):
            ...
            self.convnet = nn.Sequential(nn.Conv2d(in_channels, 32, 3, stride=2),
                                        ...
                                        nn.ReLU(), nn.Conv2d(32, 32, 3, stride=1),
                                        nn.ReLU())
            self.image_truncate = nn.Sequential(
                nn.Conv2d(32, 64, 3, stride=2),
                nn.Conv2d(64, 64, 4, stride=1),
                nn.ReLU()
            )
            if(self.env_name in task_set):
                self.tactile_truncate = nn.Sequential(
            ...
            if key == 'image':
                ...
                if(env_name in task_set):
                    self.conv3 = nn.Conv2d(encoder_dim//4, encoder_dim//2, 1)
                else:
                    self.conv3 = nn.Conv2d(encoder_dim//4, encoder_dim//2, 3, stride=1, padding=1)

            self.conv4 = nn.Conv2d(encoder_dim//2, encoder_dim, 1)
            if(env_name in task_set):
                self.conv5 = nn.Conv2d(encoder_dim, encoder_dim * 2, 1)

    def forward(self, x):
        h = x
        if(self.structure == 'drqv2'):
            # print('h shape: ', h.shape,self.key)
            h = self.convnet(h)
            if(self.key == 'image'):
                h = self.image_truncate(h)            
            elif(self.key == 'tactile' and (self.env_name in ["HandManipulateBlockRotateZFixed-v1", "HandManipulateEggRotateFixed-v1", "HandManipulatePenRotateFixed-v1", "HandManipulatePenRotate-v1", "HandManipulatePenRotateFixedInst-v1"])):
                h = self.tactile_truncate(h)
            # print('h shape: ----', h.shape)
        else:
            # print('in ppo: x shape: ----', x.shape)
            x = F.relu(self.conv1(x))
            x = F.relu(self.conv2(x))
            x = F.relu(self.conv3(x))
            h = x
            ...
            if(self.key == 'image'):
                h = self.conv4(h)
            elif(self.key == 'tactile' and (self.env_name in task_set)):
                h = self.conv4(h)
        h = h.flatten(2).transpose(1, 2)
        # print('afterward: h shape: ----', h.shape)
        # drqv2 output: [n_envs, 81(9x9), 32(out_channel)]
        return h
    ...
    def get_embeddings(self, x, eval=True, use_vision=True, use_tactile=True, key='image'):
        ...
        if(key == 'image'):
            image_tokens = self.forward(x['image'])
            # print('image_token shape: ', image_tokens.shape)
            return image_tokens # [batch_size, 64, 256](ppo) 

        if(key == 'tactile'):
            tactile_tokens_list = []
            for i in range(1,self.num_tactiles+1):
                tactile_tokens_list.append(self.forward(x['tactile'+str(i)]))
            tactile_tokens = torch.cat(tactile_tokens_list, dim=2) # [batch_size, 64, 256]
            # print('?? ', tactile_tokens.shape, self.num_tactiles)
...
class CNNExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: gym.Space, cnn_image, cnn_tactile, vae, dim_embeddings, vision_only_control, frame_stack, obses, image_only, use_vae, augment) -> None:
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        # exit(0)
        obs_image = self.cnn_image.get_embeddings(vt_torch, eval=False, use_tactile=not self.vision_only_control, key='image')
        ...
        # print('shape: ', obs_image.shape, obs_tactile.shape)
        obs_image_flatten = obs_image.reshape(obs_image.shape[0], -1)
        self.obses_image.append(obs_image_flatten)
        if(not self.image_only):
            obs_tactile_flatten = obs_tactile.reshape(obs_tactile.shape[0], -1)
            self.obses_tactile.append(obs_tactile_flatten)
        else:
            obs_tactile = torch.zeros_like(obs_image) # not use tactile
            # note that we cannot use obs_tactile = obs_image because it provides extra information (tactile contains more sparse than image)
        # print('len: ',len(self.obses_image))

        # print('shape: ', obs_image.shape, obs_tactile.shape)
        observations = torch.cat((obs_image, obs_tactile), dim=1)

        # observations = self.vit_layer.transformer(observations)
        observations = torch.mean(observations, dim=1) # TODO: why use mean?
...
class DrqPolicy(TD3Policy):
    ...
    def __init__(
        ...
        features_extractor_kwargs = {'cnn_image': cnn_image, 'cnn_tactile': cnn_tactile, 'dim_embeddings': dim_embeddings, 'vision_only_control': vision_only_control, 'frame_stack': frame_stack, 'obses': obses, 'image_only': image_only, 'augment': augment, 'vae': None, 'use_vae': False}
        ortho_init = False
    ...
    def forward(self, obs: torch.Tensor, deterministic: bool = False) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # exit(0)
        """
        obs['image'] size: [8, 4, 64, 64, 3]
        obs['tactile'] size: [8, 4, 6, 32, 32]
        """
        # print('extract_features: ', self.extract_features);exit(0)
        # print('gg obs.shape: ', obs['image'].shape,obs['tactile'].shape)

        # if self.augment:
        #     obs = random_overlay(obs)
        ...
        # print('latent? ', latent_pi.shape, latent_vf.shape)
        values = self.value_net(latent_vf)
        distribution = self._get_action_dist_from_latent(latent_pi)
        actions = distribution.get_actions(deterministic=deterministic)
        # print('actions? ', actions.shape);exit(0)
...
class CNNPolicy(ActorCriticCnnPolicy):
    ...
    def __init__(
        ...
        self.augment = augment
        features_extractor_class = CNNExtractor
        features_extractor_kwargs = {'cnn_image': cnn_image, 
...
class VTTExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: gym.Space, cnn_image, cnn_tactile, dim_embeddings, vision_only_control, frame_stack, obses, image_only, augment) -> None:
        super().__init__(observation_space, dim_embeddings)
        self.flatten = nn.Flatten()
        self.cnn_image = cnn_image
        self.cnn_tactile = cnn_tactile
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        vt_torch = vt_load(observations, frame_stack=self.frame_stack, no_assertion=True)
        if torch.cuda.is_available():
            for key in vt_torch:
                vt_torch[key] = vt_torch[key].to('cuda')
                # print('key, shape: ', key, vt_torch[key].shape)
...
class VTTPolicy(ActorCriticCnnPolicy):
    ...
    def __init__(
        ...
        self.feature_truncate = tnn.Linear(2592, 288)
...
class VTTInsertionPolicy(ActorCriticCnnPolicy):
