...
class PPO_CNN(OnPolicyAlgorithm):
    ...
    def __init__(
        ...
        self.lst_timestep = 0
        self.contrastive_switch_limit = 100 # set 100 in correspondence to Self-supervised Co-training for Video Representation Learning（Neurips’20）
        self.switch_tag = 0
    ...
    def load_cnn(self, cnn_image, cnn_tactile):
        self.cnn_image = cnn_image
        self.cnn_tactile = cnn_tactile
        self.cnn_image_optimizer = th.optim.Adam(self.cnn_image.parameters(), lr=1e-4)
        self.cnn_tactile_optimizer = th.optim.Adam(self.cnn_tactile.parameters(), lr=1e-4)
    ...
    def train(self) -> None:
        ...
        """
        # print('testttttt ', self.policy.features_extractor.frame_stack)
        # print('testttttt ', self.policy.features_extractor.obses);exit(0)
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                if(len(self.policy.features_extractor.obses_image) > 1):
                    obses_image = th.cat(self.policy.features_extractor.obses_image, dim=0)
                    obses_tactile = th.cat(self.policy.features_extractor.obses_tactile, dim=0)
                    print('obses shape: ', obses_image.shape, obses_tactile.shape)
                    if(False):  # TODO: change to following code
                    # if(len(obses_image) == 0 or self.num_timesteps <= self.contrastive_limit): # 只有将 obses 的长度不为 0 时才重新计算对比损失
                       ...
                       # TODO: not forget to switch image and tactile every (e.g. 100) times
                        obs1, obs2 = obses_image, obses_tactile
                        self.lst_timestep += 1
                ...
                if(len(self.policy.features_extractor.obses_image) > 1):
                    loss = loss_policy + self.contrastive_coef * contrastive_loss # we set contrastive_coef = 0.01
                else:
                    loss = loss_policy
                ...
                self.policy.features_extractor.obses_tactile = []

            # self.policy.features_extractor.obses = [] # test if cuda out of memory
            self._n_updates += 1
