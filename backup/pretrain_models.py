...
class InfoNCELoss(nn.Module):
    ...
    """
    def __init__(self, repr_dim, feature_dim, use_topk, topk, temp=0.1):
        super().__init__()
        self.temp = temp
...
class CNNEncoder(nn.Module):
    def __init__(self, in_channels, encoder_dim,num_tactiles,frame_stack = 1, key='image', env_name='Door', structure="drqv2"):
        ...
        if(self.structure == 'drqv2'):
            self.convnet = nn.Sequential(nn.Conv2d(in_channels, 16, 3, stride=2),
            ...
            if key == 'image':
                ...
                if env_name == "HandManipulateBlockRotateZFixed-v1" or env_name == "HandManipulateEggRotateFixed-v1" or env_name == "HandManipulatePenRotateFixed-v1":
    ...
    def forward(self, x):
        ...
        if(self.structure == 'drqv2'):
            ...
            if(self.key == 'image'):
                h = self.conv4(h)
            elif(self.key == 'tactile' and (self.env_name == "HandManipulateBlockRotateZFixed-v1" or self.env_name == "HandManipulateEggRotateFixed-v1" or self.env_name == "HandManipulatePenRotateFixed-v1")):
                h = self.conv4(h)
    ...
    def get_embeddings(self, x, eval=True, use_vision=True, use_tactile=True, key='image'):
        ...
        # print('image shape: ', image_tokens.shape) # [batch_size, 64, 256]
        # print('tactile shape: ', tactile_tokens.shape) # [batch_size, 32, 256]
        # tokens = torch.cat((image_tokens, tactile_tokens), dim=1)
        # return tokens

        # get patches
        # if use_vision:
        #     image_patches = x['image']
        #     batch, num_image_patches, *_ = image_patches.shape
        # else:
        #     image_patches = torch.zeros((x['tactile1'].shape[0], 0, 3)).to(device)
        #     num_image_patches = 0
        ...
        #     tactile_patches_list = []
        #     for i in range(1,self.num_tactiles+1):
        #         tactile_patches_list.append(x['tactile'+str(i)])
        #     tactile_patches = torch.cat(tactile_patches_list, dim=1)
        #     batch, num_tactile_patches, *_ = tactile_patches.shape
        # else:
        #     tactile_patches = torch.zeros((x['image'].shape[0], 0, 3)).to(device)
...
class CNNExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: gym.Space, cnn_image, cnn_tactile, dim_embeddings, vision_only_control, frame_stack, obses) -> None:
        super().__init__(observation_space, dim_embeddings)
        self.flatten = nn.Flatten()
...
class CNNPolicy(ActorCriticCnnPolicy):
    ...
    def __init__(
        ...
        features_extractor_kwargs = {'cnn_image': cnn_image, 'cnn_tactile': cnn_tactile, 'dim_embeddings': dim_embeddings, 'vision_only_control': vision_only_control, 'frame_stack': frame_stack, 'obses': obses}
        ortho_init = False
