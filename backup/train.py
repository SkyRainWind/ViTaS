...
from models.pretrain_models import CNNEncoder, InfoNCELoss
...
from models.pretrain_models import VTT, VTMAE, CNNPolicy
...
def main():
    ...
    parser.add_argument("--total_timesteps", type=int, default=int(6e6)) # TODO: change to 6e6
    ...
    parser.add_argument("--dim_embedding", type=int, default=32) # mlp extractor = dim_embedding
    ...
    parser.add_argument("--contrastive_limit", type=int, default=(int)(2e6))
    ...
    if(config.structure == 'ppo'):
        ...
        infonce = InfoNCELoss(repr_dim=256*64, feature_dim=1024, use_topk=config.use_topk, topk=config.topk).to("cuda")
    else:    # drqv2
        infonce = InfoNCELoss(repr_dim=2592, feature_dim=1024, use_topk=config.use_topk, topk=config.topk).to("cuda")
    ...
    if config.state_type == "vision_and_touch" or config.state_type == "touch":
        ...
        if config.env == "HandManipulateBlockRotateZFixed-v1" or config.env == "HandManipulateEggRotateFixed-v1" or config.env == "HandManipulatePenRotateFixed-v1":
            # num_tactiles = 1 # TODO: why originally set to 1 ?
            num_tactiles = 1
    ...
    cnn_tactile.initialize_training({"lr": 1e-4, "batch_size": config.mae_batch_size})
