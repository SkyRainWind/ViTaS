...
class EvalCallback(BaseCallback):
    ...
    def _on_rollout_start(self, force = False) -> None:
        self.count += 1
        if self.count >= self.eval_every or force == True:
...
def create_callbacks(config, model, num_tactiles, objects, holders):
    ...
    if config.env in ["Door", "Stack", "Lift", "LiftCan", "LiftCap", "TwoArmPegInHole", "Wipe", "TwoArmHandover", "TrueTwoArmPegInHole", "PickPlace"]:
    ...
    # uncomment following code to save model every save_freq times(1e5 for default)
    ...
    # print(default_logger.dir.split("/")[-1], project_name, config.wandb_dir, config.wandb_id, config.wandb_entity)
    wandb.init(
