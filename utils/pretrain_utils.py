...
def vt_load(
    x, image_normalization=[0, 1], tactile_normalization=[-1, 1], squeeze=False, frame_stack=1, no_assertion=False
):
    ...
    # print('?? ', x["image"].shape, x['tactile'].shape, frame_stack)
    # Preprocess the image
    if "image" in x:
        # if(not no_assertion):assert x["image"].shape[-1] == 3*frame_stack
    ...
    if "tactile" in x:
        # if(not no_assertion):assert x["tactile"].shape[1] == 3*frame_stack or x["tactile"].shape[1] == 6*frame_stack or x["tactile"].shape[1] == 12*frame_stack
...
def log_videos(
    ...
    for (x, reward) in zip(obses, rewards_per_step):  # For each timestep
        ...
        # print('image shape: ',image.shape);exit(0)
